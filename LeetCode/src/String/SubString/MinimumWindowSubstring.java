 ...
 *
 * 使用longestsubstrnorepeat中的求子串的模板方法
 *
 * Answer:
...
public class MinimumWindowSubstring {
    public static String minimumSubstring(String s, String alphabet) {
        ...
        for(int i = 0; i < alphabet.length(); i++) {
            map[alphabet.charAt(i)]++;
        }

        // begin end两个指针指向窗口的首位，d记录窗口的长度，
        // counter记录T中还有几个字符没被窗口包含
        int begin = 0, end = 0, d = Integer.MAX_VALUE, counter = alphabet.length(), head = 0;
        // end指针一直向后遍历
        while(end < s.length()) {
    ...
    public static void main(String[] args) {
        minimumSubstring("cabwefgewcwaefgcf", "cae");
    }
}
